...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, isOpen, onClose }) => {
  const navItems = [
    ...
    { id: ViewMode.INVENTORY, label: 'Master Stock', icon: 'fa-box-archive' },
    { id: ViewMode.CONFIG, label: 'System Config', icon: 'fa-sliders' },
  ];
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <div 
        className={`fixed top-0 left-0 h-screen w-64 bg-surface border-r border-border z-40 transform transition-transform duration-300 ease-in-out md:translate-x-0 flex flex-col ${
